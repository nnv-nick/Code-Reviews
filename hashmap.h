...
class HashMap {
 public:
    ...
    template <typename IterTemp>
    HashMap(IterTemp start, IterTemp finish, Hash new_hash = Hash());

    HashMap(std::initializer_list<KeyValuePair>
             to_hashmap, Hash new_hash = Hash());

    void insert(const KeyValuePair& new_elem);
...
HashMap<KeyType, ValueType, Hash>::HashMap(
        IterTemp start, IterTemp finish, Hash new_hash)
        : hasher_(new_hash) {
    hash_table_.resize(modulo_);
...
template<class KeyType, class ValueType, class Hash>
void HashMap<KeyType, ValueType, Hash>::insert(const KeyValuePair& new_elem) {
